...
### 突破 16：双重安全无窗隔离与代理毫秒级熔断防护体系 (v2.8)
* **卡点**：
  - 电脑偶尔会闪过类似于终端启动的黑色控制台窗口；且当后台调用 Antigravity 时若遇到网络波动，会触发 Google OAuth 网页授权弹窗打开 Chrome 浏览器抢焦点。
* **突破**：
  1. **双重安全无窗隔离**：子进程组合使用 `subprocess.CREATE_NO_WINDOW | 0x00000008` (DETACHED_PROCESS) 与 `hidden_startupinfo()`，彻底切断控制台宿主 `conhost.exe` 的绘制与弹窗；
  2. **严格锁定直接调用 `agy.exe`**：杜绝向 `.cmd` 批处理入口回退，从物理上消灭 `cmd.exe` 黑色控制台弹窗；
  3. **代理毫秒级预检熔断器**：在发起 Antigravity 查询前，0.08 秒极速预检本地代理端口（7897），代理不通或网络波动时坚决不拉起外部进程，直接使用本地缓存展示；
  4. **失败静默退避保护期**：若 Antigravity 查询偶发异常，自动进入 180 秒静默保护期，杜绝高频重试唤起 Chrome 授权页；手动点击刷新可随时强制触发。
...
| **v2.8** | 2026-09-09 | **当前稳定版本**：实施双重安全无窗隔离（`CREATE_NO_WINDOW` + `DETACHED_PROCESS`）；严格锁定 `agy.exe` 杜绝批处理回退；增加本地代理毫秒级连通性预检熔断器与 180s 静默退避保护期，彻底根除黑色终端窗口闪烁与 Chrome 浏览器 Google 账号登录弹窗。 |
