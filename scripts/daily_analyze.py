...
prompt_parts.append("")
prompt_parts.append("**禁止使用Markdown表格！表格在微信里会乱。用列表格式。**")
prompt_parts.append("")
prompt_parts.append("格式（严格照此格式，每个项目一行标题，有改动的下面逐条缩进）：")
prompt_parts.append("")
prompt_parts.append("**项目名** · 状态标识 — 一句话说明")
prompt_parts.append("  · v1.0 改动描述（每条独占一行，按时间正序）")
prompt_parts.append("  · v1.1 下一条改动")
prompt_parts.append("")
prompt_parts.append("示例：")
prompt_parts.append("**动物大战** · 正常 — 动物自动对战")
prompt_parts.append("  · v3.4 修复致命bug——补回useSkill()和rangedAttack()方法")
prompt_parts.append("  · v3.5 版本号更新，新增冒烟测试(19项全通过)")
prompt_parts.append("  · v3.6 防御性错误处理——loop加try-catch、NaN坐标自动重置")
prompt_parts.append("  · v4.0 新增青蛙+臭鼬+沼泽地图+卡片选择UI+引导页")
prompt_parts.append("**中国象棋** · 正常 — 双人象棋，无变动")
prompt_parts.append("")
prompt_parts.append("规则：")
prompt_parts.append("- **禁止使用表格！禁止使用br标签！**")
prompt_parts.append("- 每条 commit 独占一行，用「  · 」开头缩进，按时间正序（旧→新）")
prompt_parts.append("- 没有 commit 的项目直接写「无变动」，不要展开空列表")
prompt_parts.append("- 星月神殿永远标注「已退役」")
prompt_parts.append("")
prompt_parts.append("HTTP状态参考：")
