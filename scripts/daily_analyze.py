...
prompt_parts.append("")
prompt_parts.append("**重要：「昨日动态」必须根据下方的 git log 来判断，不要根据昨日日报推测！**")
prompt_parts.append("git log 记录了所有代码提交，是唯一可靠的数据源。仔细看每条 commit message，")
prompt_parts.append("从中提取每个项目昨天做了什么改动（版本号、功能、Bug修复等）。")
prompt_parts.append("如果 git log 里某个项目出现了多次，逐条列出。如果没出现，写'无变动'。")
prompt_parts.append("")
...
prompt_parts.append("")
prompt_parts.append("日志是**操作轨迹的事实记录**，不是评价。根据下方 git log 如实记录：")
prompt_parts.append("")
prompt_parts.append("- **代码提交**：逐条列出昨天每个 commit（项目、版本号、改动内容）")
prompt_parts.append("- **操作原因**：根据 commit message 推断每次操作的原因（修bug？新功能？调整配置？）")
prompt_parts.append("- **部署状态**：改动是否已推送部署")
prompt_parts.append("")
prompt_parts.append("如果没有 commit，写'昨日无代码提交'。")
prompt_parts.append("")

# 板块4：今日小结（评价和建议）
prompt_parts.append("## 4. 今日小结")
prompt_parts.append("")
prompt_parts.append("小结是你的**评价和建议**，不是重复日志内容：")
prompt_parts.append("")
...
prompt_parts.append("--- 最近3天 git log（代码改动的唯一真相来源） ---")
